...
module Definition.Conversion.TransitivityAux where
...
mutual
  ...
  trans~↑! {A = A} {Γ = Γ} el Γ≡Δ (cast-cong {A'} x x₁ x₂ x₃' x₄' x₅ x₆) (cast-refl A~B ⊢t ⊢u x₃ x₄) =
    let ⊢Γ , _ , _ = contextConvSubst Γ≡Δ
        t~v = transConv↓Term Γ≡Δ (univ (soundness~↓! x)) PE.refl x₄' x₃
        K , wK , XY , [U] , _  = trans~↓! PE.refl Γ≡Δ x A~B
        K'' , wK'' , A~A , [U]'' , _  = transSymR~↓! PE.refl (reflConEq ⊢Γ) XY x₁
  ...
  trans~↑! {Δ = Δ} el Γ≡Δ (cast-refl' {B = B} A~B ⊢t ⊢u x₃ x₄) (cast-cong {A' = A'} {B' = B'} x₅ x₆ x₇ x₈ x₉ x₁₀ x₁₁) =
    let ⊢Γ , ⊢Δ , _ = contextConvSubst Γ≡Δ
        t~v = transConv↓Term Γ≡Δ (refl (syntacticTerm ⊢t)) PE.refl x₃ x₉
        K , wK , A'~B , [U] , _  = transSymL~↓! PE.refl (symConEq Γ≡Δ) x₅ A~B
        K'' , wK'' , A'~B' , [U]'' , _  = trans~↓! PE.refl Γ≡Δ A~B x₆ 
        K' , wK' , A~A , [U]' , _  = transSymL~↓! PE.refl (symConEq Γ≡Δ) x₅ A'~B'
        eqU = U≡A-whnf [U] wK
        A'~BU = PE.subst (λ X →  Δ ⊢ A' ~ B ↓! X ^ ι ¹) eqU A'~B
        eqU' = U≡A-whnf [U]' wK'
    ...
    in _ , cast-refl' A~A' (conv ⊢t (stabilityEq (symConEq Γ≡Δ) (univ (soundness~↓! x₅))))
      ...
      A≡A' ,  stabilityEq (symConEq Γ≡Δ) (univ (soundness~↓! A'~BU))
  ...
  transSymR~↑! PE.refl Γ≡Δ t~u (cast-refl A~B ⊢v ⊢u (ne-ins x' x₁' x₂' ([~] K D whK u~v)) x₄) =
    let net , neu = ne~↑! t~u
        t≡u = soundness~↑! t~u
        _ , neA , neB = ne~↓! A~B
        ⊢A , ⊢t , ⊢u' = syntacticEqTerm t≡u
        _ , A≡B = neTypeEq neu ⊢u' (stabilityTerm (symConEq Γ≡Δ) ⊢u)
        _ , ⊢B = syntacticEq A≡B
        C , t~v' , A≡C , C≡B  = transSymR~↑! PE.refl Γ≡Δ t~u u~v
        _ , ⊢C = syntacticEq A≡C
        XC , wXC , DXC = whNorm ⊢C
        t[conv↑]v = ne-ins (conv ⊢t A≡B) (stabilityTerm (symConEq Γ≡Δ) ⊢v) neA ([~] _ (red DXC) wXC t~v')
   in _ , cast-refl' (stability~↓! (symConEq Γ≡Δ) A~B)
                     (conv ⊢t A≡B) 
                     (stabilityTerm (symConEq Γ≡Δ) ⊢v) t[conv↑]v (stabilityTerm (symConEq Γ≡Δ) x₄) , A≡B , stabilityEq (symConEq Γ≡Δ) (univ (soundness~↓! A~B)) 

  transSymR~↑! el Γ≡Δ (var-refl x₁ x≡y) (var-refl x₂ x≡y₁) =
    _ , var-refl x₁ (PE.trans x≡y (PE.sym x≡y₁))
    , refl (syntacticTerm x₁) , proj₂ (neTypeEq (var _) x₁ (PE.subst (λ x → _ ⊢ var x ∷ _ ^ _) (PE.trans x≡y₁ (PE.sym x≡y))
                                                (stabilityTerm (symConEq Γ≡Δ) (PE.subst (λ lx → _ ⊢ _ ∷ _ ^ [ ! , lx ]) (PE.sym el) x₂))))
  transSymR~↑! el Γ≡Δ (var-refl x x₁) (castℕ-refl x₂ x₃) = {!!}
{-
  transSymR~↑! el Γ≡Δ (app-cong x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (natrec-cong x x₁ x₂ x₃) Y = {!!}
  transSymR~↑! el Γ≡Δ (Emptyrec-cong x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-cong x x₁ x₂) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-ℕ x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-ℕ0 x) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-ℕS x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-U x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-Uℕ x) Y = {!!}
  transSymR~↑! el Γ≡Δ (Id-UΠ x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-cong x x₁ x₂ x₃ x₄ x₅ x₆) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-refl x x₁ x₂ x₃ x₄) Y = {!!}
  transSymR~↑! el Γ≡Δ (castℕ-refl x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-refl' x x₁ x₂ x₃ x₄) Y = {!!}
  transSymR~↑! el Γ≡Δ (castℕ-refl' x x₁) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-neℕ x x₁ x₂ x₃) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-ℕ x x₁ x₂ x₃) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-ℕℕ x x₁ x₂) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-neΠ x x₁ x₂ x₃ x₄) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-Π x x₁ x₂ x₃ x₄) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-Πℕ x x₁ x₂ x₃) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-ℕΠ x x₁ x₂ x₃) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-ΠΠ%! x x₁ x₂ x₃ x₄) Y = {!!}
  transSymR~↑! el Γ≡Δ (cast-ΠΠ!% x x₁ x₂ x₃ x₄) Y = {!!}
-}
  transSymR~↑! el Γ≡Δ = {!!}

  transSymL~↑! : ∀ {t u v A B Γ Δ l l'}
         → l PE.≡ l'
         → ⊢ Γ ≡ Δ
         → Γ ⊢ u ~ t ↑! A ^ l
         → Δ ⊢ u ~ v ↑! B ^ l'
         → ∃ λ C → Γ ⊢ t ~ v ↑! C ^ l × Γ ⊢ A ≡ C ^ [ ! , l ] × Γ ⊢ C ≡ B ^ [ ! , l ]
  transSymL~↑! el Γ≡Δ X Y = {!!}
  ...
  transSymR~↓! PE.refl Γ≡Δ ([~] A₁ D whnfA k~l) ([~] A₂ D₁ whnfA₁ k~l₁) = 
   let C , t~v , A≡C , C≡B = transSymR~↑! PE.refl Γ≡Δ k~l k~l₁
       ⊢C , _ = syntacticEq C≡B
       X , wX , DX = whNorm ⊢C
   in X , wX , [~] _ (red DX) wX t~v , trans (sym (subset* D)) (trans A≡C (subset* (red DX))) , trans (trans (sym (subset* (red DX))) C≡B) (subset* (stabilityRed* (symConEq Γ≡Δ) D₁))
  ...
  transSymL~↓! PE.refl Γ≡Δ ([~] A₁ D whnfA k~l) ([~] A₂ D₁ whnfA₁ k~l₁) =
   let C , t~v , A≡C , C≡B = transSymL~↑! PE.refl Γ≡Δ k~l k~l₁
       ⊢C , _ = syntacticEq C≡B
       X , wX , DX = whNorm ⊢C
   in X , wX , [~] _ (red DX) wX t~v , trans (sym (subset* D)) (trans A≡C (subset* (red DX))) , trans (trans (sym (subset* (red DX))) C≡B) (subset* (stabilityRed* (symConEq Γ≡Δ) D₁))
