...
mutual
  ...
  stabilitySize~↑! Γ≡Δ (Id-cong x x₁ x₂) = PE.cong₃ (λ a b c → 1+ (a + b + c))
             (stabilitySize~↓! Γ≡Δ x) 
             (stabilitySizeConv↑Term Γ≡Δ x₁)
             (stabilitySizeConv↑Term Γ≡Δ x₂)
  stabilitySize~↑! Γ≡Δ (Id-ℕ x x₁) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↑Term Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (Id-ℕ0 x) = PE.cong 1+ (stabilitySize~↓! Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (Id-ℕS x x₁) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySizeConv↑Term Γ≡Δ x)
             (stabilitySize~↓! Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (Id-U x x₁) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↑Term Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (Id-Uℕ x) = PE.cong 1+ (stabilitySize~↓! Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (Id-UΠ x x₁) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySizeConv↑Term Γ≡Δ x)
             (stabilitySize~↓! Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (cast-cong x x₁ _ _ x₂ _ _) =  PE.cong₃ (λ a b c → 1+ (a + b + c))
             (stabilitySize~↓! Γ≡Δ x) 
             (stabilitySize~↓! Γ≡Δ x₁)
             (stabilitySizeConv↓Term Γ≡Δ x₂)
  stabilitySize~↑! Γ≡Δ (cast-refl x _ _ x₁ _) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↓Term Γ≡Δ x₁)
  ...
  stabilitySize~↑! Γ≡Δ (cast-ℕ x x₁ x₂ x₃) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↑Term Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (cast-ℕℕ x x₁ x₂) = PE.cong 1+ (stabilitySize~↓! Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (cast-neΠ _ x x₁ x₂ x₃) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↑Term Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (cast-Π _ x x₁ x₂ x₃) = PE.cong₂ (λ a b → 1+ (a + b))
             (stabilitySize~↓! Γ≡Δ x)
             (stabilitySizeConv↑Term Γ≡Δ x₁)
  stabilitySize~↑! Γ≡Δ (cast-Πℕ _ x x₁ x₂) = PE.cong 1+ (stabilitySizeConv↑Term Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (cast-ℕΠ _ x x₁ x₂) = PE.cong 1+ (stabilitySizeConv↑Term Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (cast-ΠΠ%! _ _ x x₁ x₂) = PE.cong 1+ (stabilitySizeConv↑Term Γ≡Δ x)
  stabilitySize~↑! Γ≡Δ (cast-ΠΠ!% _ _ x x₁ x₂) = PE.cong 1+ (stabilitySizeConv↑Term Γ≡Δ x)
  ...
  stabilitySizeConv↓Term Γ≡Δ (∃-cong F A<>B A<>B₁) = PE.cong₂ (λ a b → 1 + (a + b))
                  (stabilitySizeConv↑Term Γ≡Δ A<>B)
                  (stabilitySizeConv↑Term _ A<>B₁)
