...
appTerm′ : ∀ {F G t u Γ rF rG l l′ l″}
          ([F] : Γ ⊩⟨ l″ ⟩ F ^ [ rF , TypeInfo.l rG ])
          ([G[u]] : Γ ⊩⟨ l′ ⟩ G [ u ] ^ rG)
          ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ^ rF ▹ G ^ rG)
          ([t] : Γ ⊩⟨ l ⟩ t ∷ Π F ^ rF ▹ G ^ rG / Π-intr [ΠFG])
          ([u] : Γ ⊩⟨ l″ ⟩ u ∷ F ^ [ rF , TypeInfo.l rG ] / [F])
        → Γ ⊩⟨ l′ ⟩ t ∘ u ∷ G [ u ] ^ rG / [G[u]]
appTerm′ {t = t} {Γ = Γ} {rG = [ ! , lG ]} [F] [G[u]] (noemb (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F′] [G′] G-ext))
  ...
  let ΠFG≡ΠF′G′ = whnfRed* (red D) Πₙ
      ...
      ⊢Γ = wf ⊢F
      [u]′ = irrelevanceTerm′ F≡idF′ rF≡rF′ [F] ([F′] id ⊢Γ) [u]
      [f∘u] = irrelevanceTerm″ idG′ᵤ≡Gᵤ idf∘u≡f∘u
                                ([G′] id ⊢Γ [u]′) [G[u]] ([f]₁ id ⊢Γ [u]′)
      ⊢u = escapeTerm [F] [u]
      d′ = PE.subst (λ x → Γ ⊢ t ⇒* f ∷ x ^ lG) (PE.sym ΠFG≡ΠF′G′) (redₜ d)
  in  proj₁ (redSubst*Term (app-subst* d′ ⊢u) [G[u]] [f∘u])
appTerm′ {t = t} {Γ = Γ} {rG = [ % , lG ]} [F] [G[u]] (noemb (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F′] [G′] G-ext))
         [t] [u] =
  let ⊢u = escapeTerm [F] [u]
      ⊢t = escapeTerm (Πᵣ (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F′] [G′] G-ext)) [t]
  in logRelIrr [G[u]] (⊢t ∘ⱼ  ⊢u)
appTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ι ⁰} (Nat.s≤s ⁰<¹) x) [t] [u] = appTerm′ [F] [G[u]] x [t] [u]
appTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ι ¹} (Nat.s≤s ()) x)
appTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ∞} (Nat.s≤s ()) x)
appTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ι ⁰} (Nat.s≤s ⁰<¹) x) [t] [u] = appTerm′ [F] [G[u]] x [t] [u]
appTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s ⁰<¹)) x) [t] [u] = appTerm′ [F] [G[u]] x [t] [u]
appTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ∞} (Nat.s≤s (Nat.s≤s ())) x₁)


-- Application of reducible terms.
appTerm : ∀ {F G t u Γ rF rF' rG l l′ l″} (eqr : rF PE.≡ rF')
          ([F] : Γ ⊩⟨ l″ ⟩ F ^ [ rF , TypeInfo.l rG ])
          ([G[u]] : Γ ⊩⟨ l′ ⟩ G [ u ] ^ rG)
          ([ΠFG] : Γ ⊩⟨ l ⟩ Π F ^ rF' ▹ G ^ rG)
          ([t] : Γ ⊩⟨ l ⟩ t ∷ Π F ^ rF' ▹ G ^ rG / [ΠFG])
          ([u] : Γ ⊩⟨ l″ ⟩ u ∷ F ^ [ rF , TypeInfo.l rG ] / [F])
        → Γ ⊩⟨ l′ ⟩ t ∘ u ∷ G [ u ] ^ rG / [G[u]]
...
app-congTerm′ : ∀ {F G t t′ u u′ Γ rF rG l l′}
          ([F] : Γ ⊩⟨ l′ ⟩ F ^ [ rF , TypeInfo.l rG ])
          ([G[u]] : Γ ⊩⟨ l′ ⟩ G [ u ] ^ rG)
          ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ^ rF ▹ G ^ rG)
          ([t≡t′] : Γ ⊩⟨ l ⟩ t ≡ t′ ∷ Π F ^ rF ▹ G ^ rG / Π-intr [ΠFG])
          ([u] : Γ ⊩⟨ l′ ⟩ u ∷ F ^ [ rF , TypeInfo.l rG ] / [F])
          ([u′] : Γ ⊩⟨ l′ ⟩ u′ ∷ F ^ [ rF , TypeInfo.l rG ] / [F])
          ([u≡u′] : Γ ⊩⟨ l′ ⟩ u ≡ u′ ∷ F ^ [ rF , TypeInfo.l rG ] / [F])
        → Γ ⊩⟨ l′ ⟩ t ∘ u ≡ t′ ∘ u′ ∷ G [ u ] ^ rG / [G[u]]
app-congTerm′ {F′} {G′} {t = t} {t′ = t′} {Γ = Γ} {rF = rF} {rG = [ ! , lG ]}
              [F] [G[u]] (noemb (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F]₁ [G] G-ext))
  ...
  let ΠFG≡ΠF′G′ = whnfRed* (red D) Πₙ
      ...
      wkidG₁[u′]≡G[u′] = PE.cong (λ x → x [ _ ])
                                 (PE.trans (wk-lift-id _) (PE.sym G≡G′))
      ⊢Γ = wf ⊢F
      [u]′ = irrelevanceTerm′ F≡wkidF′ rF≡rF′ [F] ([F]₁ id ⊢Γ) [a]
      [u′]′ = irrelevanceTerm′ F≡wkidF′ rF≡rF′ [F] ([F]₁ id ⊢Γ) [a′]
      [u≡u′]′ = irrelevanceEqTerm′ F≡wkidF′ rF≡rF′ [F] ([F]₁ id ⊢Γ) [a≡a′]
      [G[u′]] = irrelevance′ wkidG₁[u′]≡G[u′] ([G] id ⊢Γ [u′]′)
      [G[u≡u′]] = irrelevanceEq″ wkidG₁[u]≡G[u] wkidG₁[u′]≡G[u′]
                                  ([G] id ⊢Γ [u]′) [G[u]]
                                  (G-ext id ⊢Γ [u]′ [u′]′ [u≡u′]′)
      [f′] : Γ ⊩⟨ _ ⟩ f′ ∷ Π F′ ^ rF′ ▹ G′ ^ [ ! , lG ] / [ΠFG]
      [f′] = Πₜ f′ (idRedTerm:*: ⊢f′) funcF′ f≡f [f] [f]₁
      [g′] : Γ ⊩⟨ _ ⟩ g′ ∷ Π F′ ^ rF′ ▹ G′ ^ [ ! , lG ] / [ΠFG]
      ...
      [t′u≡t′u′] = irrelevanceEqTerm″ t∘x≡wkidt∘x′ t∘x≡wkidt∘x′ wkidG₁[u]≡G[u]
                                       ([G] id ⊢Γ [u]′) [G[u]]
                                       ([g] id ⊢Γ [u]′ [u′]′ [u≡u′]′)
      d₁ = PE.subst (λ x → Γ ⊢ t ⇒* f ∷ x ^ lG) (PE.sym ΠFG≡ΠF′G′) d
      d₂ = PE.subst (λ x → Γ ⊢ t′ ⇒* g ∷ x ^ lG) (PE.sym ΠFG≡ΠF′G′) d′
...
app-congTerm′ {F′} {G′} {t = t} {t′ = t′} {Γ = Γ} {rF = rF} {rG = [ % , lG ]}
  ...
  let ΠFG≡ΠF′G′ = whnfRed* (red D) Πₙ
      ...
      wkidG₁[u′]≡G[u′] = PE.cong (λ x → x [ _ ])
                                 (PE.trans (wk-lift-id _) (PE.sym G≡G′))
      ⊢Γ = wf ⊢F
      [u]′ = irrelevanceTerm′ F≡wkidF′ rF≡rF′ [F] ([F′] id ⊢Γ) [u]
      [u′]′ = irrelevanceTerm′ F≡wkidF′ rF≡rF′ [F] ([F′] id ⊢Γ) [u′]
      [u≡u′]′ = irrelevanceEqTerm′ F≡wkidF′ rF≡rF′ [F] ([F′] id ⊢Γ) [u≡u′]
      [G[u′]] = irrelevance′ wkidG₁[u′]≡G[u′] ([G′] id ⊢Γ [u′]′)
      [G[u≡u′]] = irrelevanceEq″ wkidG₁[u]≡G[u] wkidG₁[u′]≡G[u′]
                                  ([G′] id ⊢Γ [u]′) [G[u]]
                                  (G-ext id ⊢Γ [u]′ [u′]′ [u≡u′]′)
...
app-congTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ι ⁰} (Nat.s≤s l<) x) [t≡t′] [u] [u′] [u≡u′] = app-congTerm′ [F] [G[u]] x [t≡t′] [u] [u′] [u≡u′]
app-congTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ι ¹} (Nat.s≤s ()) x) [t≡t′] [u] [u′] [u≡u′]
app-congTerm′ {l = ι ¹} [F] [G[u]] (emb {l′ = ∞} (Nat.s≤s ()) x) [t≡t′] [u] [u′] [u≡u′]
app-congTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ι ⁰} (Nat.s≤s l<) x) [t≡t′] [u] [u′] [u≡u′] = app-congTerm′ [F] [G[u]] x [t≡t′] [u] [u′] [u≡u′]
app-congTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s l<)) x) [t≡t′] [u] [u′] [u≡u′] = app-congTerm′ [F] [G[u]] x [t≡t′] [u] [u′] [u≡u′]
app-congTerm′ {l = ∞} [F] [G[u]] (emb {l′ = ∞} (Nat.s≤s (Nat.s≤s ())) x) [t≡t′] [u] [u′] [u≡u′]
