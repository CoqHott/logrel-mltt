...
mutual
  ...
  convTermT₁ {r = [ % , ll ]} (ne (ne K D neK K≡K) (ne K₁ D₁ neK₁ K≡K₁)) (ne₌ M D′ neM K≡M)
             (neₜ d) = let K≡K₁ = PE.subst (λ x → _ ⊢ _ ≡ x ^ _)
                                          (whrDet* (red D′ , ne neM) (red D₁ , ne neK₁))
                                          (≅-eq (~-to-≅ K≡M))
                       in neₜ (conv d (reduction  (red D) (red D₁) K≡K₁))
  convTermT₁ {Γ = Γ} {r = [ ! , ι ll ]} (Πᵥ (Πᵣ rF lF lG _ _ F G D _ ⊢F ⊢G A≡A [F] [G] G-ext)
  ...
  convTermT₁ {Γ = Γ} {r = [ % , ι ll ]} (Πirrᵥ (Πirrᵣ rF lF F G D _ ⊢F ⊢G A≡A)
                         (Πirrᵣ rF₁ lF₁ F₁ G₁ D₁ nf ⊢F₁ ⊢G₁ A≡A₁))
             (Πirr₌ F′ G′ D′ nf' A≡B)
             d = let ΠF₁G₁≡ΠF′G′   = whrDet* (red D₁ , nf) (D′ , nf')
                     F₁≡F′ , rF₁≡rF′ , lF₁≡lF′ , G₁≡G′ , lG₁≡lG′ = Π-PE-injectivity ΠF₁G₁≡ΠF′G′
                     ΠFG≡ΠF₁G₁ = PE.subst (λ x → Γ ⊢ Π F ^ rF ° lF ▹ G ° ⁰ ° ll ^ % ≡ x ^ [ % , ι ll ]) (PE.sym ΠF₁G₁≡ΠF′G′)
                                          (≅-eq A≡B)
                 in conv d ΠFG≡ΠF₁G₁
  convTermT₁ {Γ = Γ} {r = [ % , ll ]} (Idᵥ (Idᵣ F t u _ D _ ⊢F ⊢t ⊢u A≡A)
                         (Idᵣ F₁ t' u' _ D₁ nf ⊢F₁ ⊢t' ⊢u' A≡A₁))
             (Id₌ F′ t′ u′ D′  nf' A≡B)
             d = let IdF₁G₁≡IdF′G′   = whrDet* (red D₁ , nf) (D′ , nf')
  ...
  convEqTermT₁ {r = [ % , ll ]} (ne (ne K D neK K≡K) (ne K₁ D₁ neK₁ K≡K₁)) (ne₌ M D′ neM K≡M)
    ...
    let K≡K₁ = PE.subst (λ x → _ ⊢ _ ≡ x ^ _)
                        ...
                        (≅-eq (~-to-≅ K≡M))
    in neₜ₌ (conv d (reduction  (red D) (red D₁) K≡K₁))
            (conv d′ (reduction  (red D) (red D₁) K≡K₁))
  convEqTermT₁ {Γ = Γ} {r = [ ! , ι ll ]} (Πᵥ (Πᵣ rF lF lG lF≤ lG≤ F G D nfD ⊢F ⊢G A≡A [F] [G] G-ext)
                           (Πᵣ rF₁ lF₁ lG₁ lF₁≤ lG₁≤ F₁ G₁ D₁ nf ⊢F₁ ⊢G₁ A≡A₁ [F]₁ [G]₁ G-ext₁))
               (Π₌ F′ G′ D′ nf' A≡B [F≡F′] [G≡G′])
               (Πₜ₌ f g d d′ funcF funcG t≡u [t] [u] [t≡u]) =
    let [A] = Πᵣ′ rF lF lG lF≤ lG≤ F G D nfD ⊢F ⊢G A≡A [F] [G] G-ext
        [B] = Πᵣ′ rF₁ lF₁ lG₁ lF₁≤ lG₁≤ F₁ G₁ D₁ nf ⊢F₁ ⊢G₁ A≡A₁ [F]₁ [G]₁ G-ext₁
        [A≡B] = Π₌ F′ G′ D′ nf' A≡B [F≡F′] [G≡G′]
        ΠF₁G₁≡ΠF′G′ = whrDet* (red D₁ , nf) (D′ , nf')
        ΠFG≡ΠF₁G₁ = PE.subst (λ x → Γ ⊢ Π F ^ rF ° lF ▹ G ° lG ° ll ^ ! ≡ x ^ [ ! , ι ll ])
                             (PE.sym ΠF₁G₁≡ΠF′G′) (≅-eq A≡B)
    in  Πₜ₌ f g (convRed:*: d ΠFG≡ΠF₁G₁) (convRed:*: d′ ΠFG≡ΠF₁G₁)
            ...
            (λ {ρ} [ρ] ⊢Δ [a] →
               let F₁≡F′ , rF₁≡rF′ , lF₁≡lF′ , G₁≡G′ , lG₁≡lG′ , lΠ≡lΠ₁ = Π-PE-injectivity ΠF₁G₁≡ΠF′G′
               ...
               in  convEqTerm₁′ PE.refl (PE.cong ι (PE.sym lG₁≡lG′)) ([G] [ρ] ⊢Δ [a]₁) ([G]₁ [ρ] ⊢Δ [a])
                               [G≡G₁] ([t≡u] [ρ] ⊢Δ [a]₁))
  convEqTermT₁ {Γ = Γ} {r = [ % , ι ll ]} (Πirrᵥ (Πirrᵣ rF lF F G D _ ⊢F ⊢G A≡A)
                           (Πirrᵣ rF₁ lF₁ F₁ G₁ D₁ nf ⊢F₁ ⊢G₁ A≡A₁))
               (Πirr₌ F′ G′ D′ nf' A≡B)
               (d , d′) = let ΠF₁G₁≡ΠF′G′ = whrDet* (red D₁ , nf) (D′ , nf')
                              ΠFG≡ΠF₁G₁ = PE.subst (λ x → Γ ⊢ Π F ^ rF ° lF ▹ G ° ⁰ ° ll ^ % ≡ x ^ [ % , ι ll ])
                                                   (PE.sym ΠF₁G₁≡ΠF′G′) (≅-eq A≡B)
                          in conv d ΠFG≡ΠF₁G₁ , conv d′ ΠFG≡ΠF₁G₁
  convEqTermT₁ {Γ = Γ} {r = [ % , ll ]} (Idᵥ (Idᵣ F t u _ D _ ⊢F ⊢t ⊢u A≡A)
                           (Idᵣ F₁ t' u' _ D₁ nf ⊢F₁ ⊢t' ⊢u' A≡A₁))
               (Id₌ F′ t′ u′ D′ nf' A≡B)
               (d , d′) = let IdF₁G₁≡IdF′G′ = whrDet* (red D₁ , nf) (D′ , nf')
                              IdFG≡IdF₁G₁ = PE.subst (λ x → Γ ⊢ Id F t u ≡ x ^ [ % , ll ])
                                                   (PE.sym IdF₁G₁≡IdF′G′) (≅-eq A≡B)
