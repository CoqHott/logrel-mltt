...
mutual
  ...
  fundamentalTermEq {Γ} (Id-ℕ-00 ⊢Γ) =
    let ⊢Unit : Γ ⊢ Unit ∷ (SProp ⁰) ^ [ ! , next ⁰ ]
        ⊢Unit = Πⱼ ≡is≤ PE.refl ▹ ≡is≤ PE.refl ▹ Emptyⱼ ⊢Γ ▹ Emptyⱼ (⊢Γ ∙ univ (Emptyⱼ ⊢Γ))
        [Γ] = valid ⊢Γ
        [SProp] = Uᵛ emb< [Γ]
        [Empty] = Emptyᵛ {ll = ⁰} [Γ]
        ...
        [Unit] = maybeEmbTermᵛ {A = SProp _} {t = Unit} [Γ] [SProp]
  ...
  fundamentalTermEq (cast-cong x x₁ x₂) = {!!}
