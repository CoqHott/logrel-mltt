...
module cast-ΠΠ-lemmas-2
       ...
       (extrecursor : ∀ {ρ Δ x x′ y y′ t t′ e e′}
        ...
        -- proj₁ ([castext] ⊢Δ ([G] [ρ] ⊢Δ [b₁]) ([G] [ρ] ⊢Δ [b₂]) (G-ext [ρ] ⊢Δ [b₁] [b₂] [b₁≡b₂])
...
b = λ F F₁ e x → cast ⁰ F₁ F (Idsym (U ⁰) F F₁ e) x
...
[b] : ∀ {F F₁ Δ} →
      ...
      (recursor : ∀ {x e}
        ([x] : Δ ⊩⟨ ι ⁰ ⟩ x ∷ F₁ ^ [ ! , ι ⁰ ] / [F₁])
        (⊢e : Δ ⊢ e ∷ Id (U ⁰) F₁ F ^ [ % , ι ¹ ])
        → Δ ⊩⟨ ι ⁰ ⟩ cast ⁰ F₁ F e x ∷ F ^ [ ! , ι ⁰ ] / [F])
      → ∀ {x e}
      → (⊢e : Δ ⊢ e ∷ Id (U ⁰) F F₁ ^ [ % , ι ¹ ])
      → ([x] : Δ ⊩⟨ ι ⁰ ⟩ x ∷ F₁ ^ [ ! , ι ⁰ ] / [F₁])
      → Δ ⊩⟨ ι ⁰ ⟩ b F F₁ e x ∷ F ^ [ ! , ι ⁰ ] / [F]
[b] ⊢Δ [F] [F₁] recursor ⊢e [x] =
  let
    ⊢e′ = Idsymⱼ (univ 0<1 ⊢Δ) (un-univ (escape [F]))
      (un-univ (escape [F₁])) ⊢e
  in recursor [x] ⊢e′
