...
substS : ∀ {F G t Γ sF sG l} ([Γ] : ⊩ᵛ Γ)
         ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
         ([G] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ⦂ sG / [Γ] ∙ [F])
         ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ⦂ sF / [Γ] / [F])
       → Γ ⊩ᵛ⟨ l ⟩ G [ t ] ⦂ sG / [Γ]
...
substSEq : ∀ {F F′ G G′ t t′ sF sG Γ l} ([Γ] : ⊩ᵛ Γ)
           ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
           ([F′] : Γ ⊩ᵛ⟨ l ⟩ F′ ⦂ sF / [Γ])
           ([F≡F′] : Γ ⊩ᵛ⟨ l ⟩ F ≡ F′ ⦂ sF / [Γ] / [F])
           ([G] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ⦂ sG / [Γ] ∙ [F])
           ([G′] : Γ ∙ F′ ⦂ sF ⊩ᵛ⟨ l ⟩ G′ ⦂ sG / [Γ] ∙ [F′])
           ([G≡G′] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ≡ G′ ⦂ sG / [Γ] ∙ [F] / [G])
           ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ⦂ sF / [Γ] / [F])
           ([t′] : Γ ⊩ᵛ⟨ l ⟩ t′ ∷ F′ ⦂ sF / [Γ] / [F′])
           ([t≡t′] : Γ ⊩ᵛ⟨ l ⟩ t ≡ t′ ∷ F ⦂ sF / [Γ] / [F])
         → Γ ⊩ᵛ⟨ l ⟩ G [ t ] ≡ G′ [ t′ ] ⦂ sG / [Γ]
...
substSEq {F} {F′} {G} {G′} {t} {t′}
  ...
  let Geq = substConsId G
      ...
      G′[t] = (proj₁ ([G′] ⊢Δ ([σ] , proj₁ ([t]′ ⊢Δ [σ]))))
      G[t]≡G′[t] = irrelevanceEq′ Geq PE.refl G[t] G[t]′
                                  ([G≡G′] ⊢Δ ([σ] , proj₁ ([t] ⊢Δ [σ])))
...
substSTerm : ∀ {F G t f Γ sF sG l} ([Γ] : ⊩ᵛ Γ)
             ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
             ([G] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ⦂ sG / [Γ] ∙ [F])
             ([f] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ f ∷ G ⦂ sG / [Γ] ∙ [F] / [G])
             ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ⦂ sF / [Γ] / [F])
           → Γ ⊩ᵛ⟨ l ⟩ f [ t ] ∷ G [ t ] ⦂ sG / [Γ]
                      / substS {F} {G} {t} [Γ] [F] [G] [t]
...
subst↑S : ∀ {F G t Γ sF sG l} ([Γ] : ⊩ᵛ Γ)
          ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
          ([G] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ⦂ sG / [Γ] ∙ [F])
          ([t] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ t ∷ wk1 F ⦂ sF / [Γ] ∙ [F]
                              / wk1ᵛ {F} {F} [Γ] [F] [F])
        → Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G [ t ]↑ ⦂ sG / [Γ] ∙ [F]
subst↑S {F} {G} {t} [Γ] [F] [G] [t] {σ = σ} ⊢Δ [σ] =
  let [wk1F] = wk1ᵛ {F} {F} [Γ] [F] [F]
      [σwk1F] = proj₁ ([wk1F] {σ = σ} ⊢Δ [σ])
      [σwk1F]′ = proj₁ ([F] {σ = tail σ} ⊢Δ (proj₁ [σ]))
      [t]′ = irrelevanceTerm′ (subst-wk F) PE.refl [σwk1F] [σwk1F]′ (proj₁ ([t] ⊢Δ [σ]))
      G[t] = proj₁ ([G] {σ = consSubst (tail σ) (subst σ t)} ⊢Δ
                               (proj₁ [σ] , [t]′))
      G[t]′ = irrelevance′ (substConsTailId {G} {t} {σ}) G[t]
  in  G[t]′
  ,   (λ {σ′} [σ′] [σ≡σ′] →
         let [σ′t] = irrelevanceTerm′ (subst-wk F) PE.refl
                                      (proj₁ ([wk1F] {σ = σ′} ⊢Δ [σ′]))
                                      (proj₁ ([F] ⊢Δ (proj₁ [σ′])))
                                      (proj₁ ([t] ⊢Δ [σ′]))
             [σt≡σ′t] = irrelevanceEqTerm′ (subst-wk F) PE.refl [σwk1F] [σwk1F]′
                                           (proj₂ ([t] ⊢Δ [σ]) [σ′] [σ≡σ′])
...
subst↑SEq : ∀ {F G G′ t t′ Γ sF sG l} ([Γ] : ⊩ᵛ Γ)
            ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
            ([G] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ⦂ sG / [Γ] ∙ [F])
            ([G′] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G′ ⦂ sG / [Γ] ∙ [F])
            ([G≡G′] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G ≡ G′ ⦂ sG / [Γ] ∙ [F] / [G])
            ...
            ([t′] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ t′ ∷ wk1 F ⦂ sF / [Γ] ∙ [F]
                                 / wk1ᵛ {F} {F} [Γ] [F] [F])
            ([t≡t′] : Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ t ≡ t′ ∷ wk1 F ⦂ sF / [Γ] ∙ [F]
                                   / wk1ᵛ {F} {F} [Γ] [F] [F])
          → Γ ∙ F ⦂ sF ⊩ᵛ⟨ l ⟩ G [ t ]↑ ≡ G′ [ t′ ]↑ ⦂ sG / [Γ] ∙ [F]
...
subst↑SEq {F} {G} {G′} {t} {t′}
  ...
  let [wk1F] = wk1ᵛ {F} {F} [Γ] [F] [F]
      ...
      [t]′ = irrelevanceTerm′ (subst-wk F) PE.refl [σwk1F] [σwk1F]′ (proj₁ ([t] ⊢Δ [σ]))
      [t′]′ = irrelevanceTerm′ (subst-wk F) PE.refl [σwk1F] [σwk1F]′ (proj₁ ([t′] ⊢Δ [σ]))
      [t≡t′]′ = irrelevanceEqTerm′ (subst-wk F) PE.refl [σwk1F] [σwk1F]′ ([t≡t′] ⊢Δ [σ])
...
substSΠ₁′ : ∀ {F G t Γ sF sG l l′}
           ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ⦂ sF ▹ G ⦂ sG)
           ([F] : Γ ⊩⟨ l′ ⟩ F ⦂ sF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ⦂ sF / [F])
         → Γ ⊩⟨ l ⟩ G [ t ] ⦂ sG
substSΠ₁′ {t = t} (noemb (Πᵣ sF′ F G D ⊢F ⊢G A≡A [F] [G] G-ext)) [F]₁ [t] =
  let F≡F′ , sF≡sF′ , G≡G′ = Π-PE-injectivity (whnfRed* (red D) Πₙ)
      ...
      ⊢Γ = wf (escape [F]₁)
      [t]′ = irrelevanceTerm′ Feq sF≡sF′ [F]₁ ([F] id ⊢Γ) [t]
...
substSΠ₂′ : ∀ {F F′ G G′ t t′ Γ sF sG l l′ l″ l‴}
           ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ⦂ sF ▹ G ⦂ sG)
           ([ΠFG≡ΠF′G′] : Γ ⊩⟨ l ⟩ Π F ⦂ sF ▹ G ≡ Π F′ ⦂ sF ▹ G′ ⦂ sG / Π-intr [ΠFG])
           ([F] : Γ ⊩⟨ l′ ⟩ F ⦂ sF)
           ([F′] : Γ ⊩⟨ l′ ⟩ F′ ⦂ sF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ⦂ sF / [F])
           ([t′] : Γ ⊩⟨ l′ ⟩ t′ ∷ F′ ⦂ sF / [F′])
           ...
           ([G′[t′]] : Γ ⊩⟨ l‴ ⟩ G′ [ t′ ] ⦂ sG)
         → Γ ⊩⟨ l″ ⟩ G [ t ] ≡ G′ [ t′ ] ⦂ sG / [G[t]]
substSΠ₂′ (noemb (Πᵣ sF′ F G D ⊢F ⊢G A≡A [F] [G] G-ext))
          (Π₌ F″ G″ D′ A≡B [F≡F′] [G≡G′])
          [F]₁ [F′] [t] [t′] [t≡t′] [G[t]] [G′[t′]] =
  let F≡F′ , sF≡sF′ ,  G≡G′ = Π-PE-injectivity (whnfRed* (red D) Πₙ)
      F′≡F″ , sF′≡sF″ , G′≡G″ = Π-PE-injectivity (whnfRed* D′ Πₙ)
      Feq = PE.trans F≡F′ (PE.sym (wk-id _))
      ...
      ⊢Γ = wf (escape [F]₁)
      [t]′ = irrelevanceTerm′ Feq sF≡sF′ [F]₁ ([F] id ⊢Γ) [t]
      [t′]′ = convTerm₂′ F′eq (PE.sym sF≡sF′) ([F] id ⊢Γ) [F′] ([F≡F′] id ⊢Γ) [t′]
      [t≡t′]′ = irrelevanceEqTerm′ Feq sF≡sF′ [F]₁ ([F] id ⊢Γ) [t≡t′]
      [Gt≡Gt′] = G-ext id ⊢Γ [t]′ [t′]′ [t≡t′]′
      [Gt′≡G′t′] = [G≡G′] id ⊢Γ [t′]′
  in  irrelevanceEq′ Geq PE.refl ([G] id ⊢Γ [t]′) [G[t]]
        (transEq′ PE.refl Geq′ PE.refl PE.refl ([G] id ⊢Γ [t]′) ([G] id ⊢Γ [t′]′)
                  [G′[t′]] [Gt≡Gt′] [Gt′≡G′t′])
...
substSΠ₂ : ∀ {F F′ G G′ t t′ Γ sF sG l l′ l″ l‴}
           ([ΠFG] : Γ ⊩⟨ l ⟩ Π F ⦂ sF ▹ G ⦂ sG)
           ([ΠFG≡ΠF′G′] : Γ ⊩⟨ l ⟩ Π F ⦂ sF ▹ G ≡ Π F′ ⦂ sF ▹ G′ ⦂ sG / [ΠFG])
           ([F] : Γ ⊩⟨ l′ ⟩ F ⦂ sF)
           ([F′] : Γ ⊩⟨ l′ ⟩ F′ ⦂ sF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ⦂ sF / [F])
           ([t′] : Γ ⊩⟨ l′ ⟩ t′ ∷ F′ ⦂ sF / [F′])
...
substSΠEq : ∀ {F G F′ G′ t u Γ sF sG l}
            ([Γ] : ⊩ᵛ Γ)
            ([F] : Γ ⊩ᵛ⟨ l ⟩ F ⦂ sF / [Γ])
            ([F′] : Γ ⊩ᵛ⟨ l ⟩ F′ ⦂ sF / [Γ])
            ([ΠFG] : Γ ⊩ᵛ⟨ l ⟩ Π F ⦂ sF ▹ G ⦂ sG / [Γ])
            ([ΠF′G′] : Γ ⊩ᵛ⟨ l ⟩ Π F′ ⦂ sF ▹ G′ ⦂ sG / [Γ])
            ([ΠFG≡ΠF′G′] : Γ ⊩ᵛ⟨ l ⟩ Π F ⦂ sF ▹ G ≡ Π F′ ⦂ sF ▹ G′ ⦂ sG / [Γ] / [ΠFG])
            ([t]   : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ⦂ sF / [Γ] / [F])
            ([u]   : Γ ⊩ᵛ⟨ l ⟩ u ∷ F′ ⦂ sF / [Γ] / [F′])
            ([t≡u] : Γ ⊩ᵛ⟨ l ⟩ t ≡ u ∷ F ⦂ sF / [Γ] / [F])
          → Γ ⊩ᵛ⟨ l ⟩ G [ t ] ≡ G′ [ u ] ⦂ sG / [Γ]
                    / substSΠ {F} {G} {t} [Γ] [F] [ΠFG] [t]
substSΠEq {F} {G} {F′} {G′} {t} {u} [Γ] [F] [F′] [ΠFG] [ΠF′G′] [ΠFG≡ΠF′G′]
           [t] [u] [t≡u] {Δ = Δ} {σ = σ} ⊢Δ [σ] =
  let [σΠFG] = proj₁ ([ΠFG] ⊢Δ [σ])
      _ , Πᵣ sF₁ F₁ G₁ D₁ ⊢F₁ ⊢G₁ A≡A₁ [F]₁ [G]₁ G-ext₁ = extractMaybeEmb (Π-elim [σΠFG])
      F≡F₁ , sF≡sF₁ , G≡G₁ = Π-PE-injectivity (whnfRed* (red D₁) Πₙ)
      [σΠF′G′] = proj₁ ([ΠF′G′] ⊢Δ [σ])
      _ , Πᵣ sF₂ F₂ G₂ D₂ ⊢F₂ ⊢G₂ A≡A₂ [F]₂ [G]₂ G-ext₂ = extractMaybeEmb (Π-elim [σΠF′G′])
      F′≡F₂ , sF′≡sF₂ ,  G′≡G₂ = Π-PE-injectivity (whnfRed* (red D₂) Πₙ)
      [σF] = proj₁ ([F] ⊢Δ [σ])
      [σF′] = proj₁ ([F′] ⊢Δ [σ])
      [σt] = proj₁ ([t] ⊢Δ [σ])
      [σu] = proj₁ ([u] ⊢Δ [σ])
      [σt]′ = irrelevanceTerm′ (PE.trans F≡F₁ (PE.sym (wk-id F₁))) sF≡sF₁
                               [σF] ([F]₁ id ⊢Δ) [σt]
      [σu]′ = irrelevanceTerm′ (PE.trans F′≡F₂ (PE.sym (wk-id F₂))) sF′≡sF₂
                               [σF′] ([F]₂ id ⊢Δ) [σu]
      [σt≡σu] = [t≡u] ⊢Δ [σ]
