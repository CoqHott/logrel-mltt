...
substSEq : ∀ {F F′ G G′ t t′ rF rG Γ l} ([Γ] : ⊩ᵛ Γ)
           ([F] : Γ ⊩ᵛ⟨ l ⟩ F ^ rF / [Γ])
           ([F′] : Γ ⊩ᵛ⟨ l ⟩ F′ ^ rF / [Γ])
           ([F≡F′] : Γ ⊩ᵛ⟨ l ⟩ F ≡ F′ ^ rF / [Γ] / [F])
           ([G] : Γ ∙ F ^ rF ⊩ᵛ⟨ l ⟩ G ^ rG / [Γ] ∙ [F])
           ([G′] : Γ ∙ F′ ^ rF ⊩ᵛ⟨ l ⟩ G′ ^ rG / [Γ] ∙ [F′])
           ([G≡G′] : Γ ∙ F ^ rF ⊩ᵛ⟨ l ⟩ G ≡ G′ ^ rG / [Γ] ∙ [F] / [G])
           ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ^ rF / [Γ] / [F])
           ([t′] : Γ ⊩ᵛ⟨ l ⟩ t′ ∷ F′ ^ rF / [Γ] / [F′])
           ([t≡t′] : Γ ⊩ᵛ⟨ l ⟩ t ≡ t′ ∷ F ^ rF / [Γ] / [F])
         → Γ ⊩ᵛ⟨ l ⟩ G [ t ] ≡ G′ [ t′ ] ^ rG / [Γ]
...
substSTerm : ∀ {F G t f Γ rF rG l} ([Γ] : ⊩ᵛ Γ)
             ([F] : Γ ⊩ᵛ⟨ l ⟩ F ^ rF / [Γ])
             ([G] : Γ ∙ F ^ rF ⊩ᵛ⟨ l ⟩ G ^ rG / [Γ] ∙ [F])
             ([f] : Γ ∙ F ^ rF ⊩ᵛ⟨ l ⟩ f ∷ G ^ rG / [Γ] ∙ [F] / [G])
             ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ^ rF / [Γ] / [F])
           → Γ ⊩ᵛ⟨ l ⟩ f [ t ] ∷ G [ t ] ^ rG / [Γ]
                      / substS {F} {G} {t} [Γ] [F] [G] [t]
...
substSΠ₁′ : ∀ {F G t Γ rF rG l l′}
           ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ^ rF ▹ G ^ rG)
           ([F] : Γ ⊩⟨ l′ ⟩ F ^ rF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ^ rF / [F])
         → Γ ⊩⟨ l ⟩ G [ t ] ^ rG
substSΠ₁′ {t = t} (noemb (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F] [G] G-ext)) [F]₁ [t] =
  let F≡F′ , rF≡rF′ , G≡G′ = Π-PE-injectivity (whnfRed* (red D) Πₙ)
      ...
      ⊢Γ = wf (escape [F]₁)
      [t]′ = irrelevanceTerm′ Feq rF≡rF′ [F]₁ ([F] id ⊢Γ) [t]
...
substSΠ₂′ : ∀ {F F′ G G′ t t′ Γ rF rG l l′ l″ l‴}
           ([ΠFG] : Γ ⊩⟨ l ⟩Π Π F ^ rF ▹ G ^ rG)
           ([ΠFG≡ΠF′G′] : Γ ⊩⟨ l ⟩ Π F ^ rF ▹ G ≡ Π F′ ^ rF ▹ G′ ^ rG / Π-intr [ΠFG])
           ([F] : Γ ⊩⟨ l′ ⟩ F ^ rF)
           ([F′] : Γ ⊩⟨ l′ ⟩ F′ ^ rF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ^ rF / [F])
           ([t′] : Γ ⊩⟨ l′ ⟩ t′ ∷ F′ ^ rF / [F′])
           ...
           ([G′[t′]] : Γ ⊩⟨ l‴ ⟩ G′ [ t′ ] ^ rG)
         → Γ ⊩⟨ l″ ⟩ G [ t ] ≡ G′ [ t′ ] ^ rG / [G[t]]
substSΠ₂′ (noemb (Πᵣ rF′ F G D ⊢F ⊢G A≡A [F] [G] G-ext))
          (Π₌ F″ G″ D′ A≡B [F≡F′] [G≡G′])
          [F]₁ [F′] [t] [t′] [t≡t′] [G[t]] [G′[t′]] =
  let F≡F′ , rF≡rF′ ,  G≡G′ = Π-PE-injectivity (whnfRed* (red D) Πₙ)
      F′≡F″ , rF′≡rF″ , G′≡G″ = Π-PE-injectivity (whnfRed* D′ Πₙ)
      Feq = PE.trans F≡F′ (PE.sym (wk-id _))
      ...
      ⊢Γ = wf (escape [F]₁)
      [t]′ = irrelevanceTerm′ Feq rF≡rF′ [F]₁ ([F] id ⊢Γ) [t]
      [t′]′ = convTerm₂′ F′eq (PE.sym rF≡rF′) ([F] id ⊢Γ) [F′] ([F≡F′] id ⊢Γ) [t′]
      [t≡t′]′ = irrelevanceEqTerm′ Feq rF≡rF′ [F]₁ ([F] id ⊢Γ) [t≡t′]
...
substSΠ₂ : ∀ {F F′ G G′ t t′ Γ rF rG l l′ l″ l‴}
           ([ΠFG] : Γ ⊩⟨ l ⟩ Π F ^ rF ▹ G ^ rG)
           ([ΠFG≡ΠF′G′] : Γ ⊩⟨ l ⟩ Π F ^ rF ▹ G ≡ Π F′ ^ rF ▹ G′ ^ rG / [ΠFG])
           ([F] : Γ ⊩⟨ l′ ⟩ F ^ rF)
           ([F′] : Γ ⊩⟨ l′ ⟩ F′ ^ rF)
           ([t] : Γ ⊩⟨ l′ ⟩ t ∷ F ^ rF / [F])
           ([t′] : Γ ⊩⟨ l′ ⟩ t′ ∷ F′ ^ rF / [F′])
...
substSΠEq : ∀ {F G F′ G′ t u Γ rF rG l}
            ([Γ] : ⊩ᵛ Γ)
            ([F] : Γ ⊩ᵛ⟨ l ⟩ F ^ rF / [Γ])
            ([F′] : Γ ⊩ᵛ⟨ l ⟩ F′ ^ rF / [Γ])
            ([ΠFG] : Γ ⊩ᵛ⟨ l ⟩ Π F ^ rF ▹ G ^ rG / [Γ])
            ([ΠF′G′] : Γ ⊩ᵛ⟨ l ⟩ Π F′ ^ rF ▹ G′ ^ rG / [Γ])
            ([ΠFG≡ΠF′G′] : Γ ⊩ᵛ⟨ l ⟩ Π F ^ rF ▹ G ≡ Π F′ ^ rF ▹ G′ ^ rG / [Γ] / [ΠFG])
            ([t]   : Γ ⊩ᵛ⟨ l ⟩ t ∷ F ^ rF / [Γ] / [F])
            ([u]   : Γ ⊩ᵛ⟨ l ⟩ u ∷ F′ ^ rF / [Γ] / [F′])
            ([t≡u] : Γ ⊩ᵛ⟨ l ⟩ t ≡ u ∷ F ^ rF / [Γ] / [F])
          → Γ ⊩ᵛ⟨ l ⟩ G [ t ] ≡ G′ [ u ] ^ rG / [Γ]
...
substSΠEq {F} {G} {F′} {G′} {t} {u} [Γ] [F] [F′] [ΠFG] [ΠF′G′] [ΠFG≡ΠF′G′]
  ...
  let [σΠFG] = proj₁ ([ΠFG] ⊢Δ [σ])
      ...
      F≡F₁ , rF≡rF₁ , G≡G₁ = Π-PE-injectivity (whnfRed* (red D₁) Πₙ)
      [σΠF′G′] = proj₁ ([ΠF′G′] ⊢Δ [σ])
      _ , Πᵣ rF₂ F₂ G₂ D₂ ⊢F₂ ⊢G₂ A≡A₂ [F]₂ [G]₂ G-ext₂ = extractMaybeEmb (Π-elim [σΠF′G′])
      F′≡F₂ , rF′≡rF₂ ,  G′≡G₂ = Π-PE-injectivity (whnfRed* (red D₂) Πₙ)
      [σF] = proj₁ ([F] ⊢Δ [σ])
      [σF′] = proj₁ ([F′] ⊢Δ [σ])
      [σt] = proj₁ ([t] ⊢Δ [σ])
      [σu] = proj₁ ([u] ⊢Δ [σ])
      [σt]′ = irrelevanceTerm′ (PE.trans F≡F₁ (PE.sym (wk-id F₁))) rF≡rF₁
                               [σF] ([F]₁ id ⊢Δ) [σt]
      [σu]′ = irrelevanceTerm′ (PE.trans F′≡F₂ (PE.sym (wk-id F₂))) rF′≡rF₂
                               [σF′] ([F]₂ id ⊢Δ) [σu]
      [σt≡σu] = [t≡u] ⊢Δ [σ]
