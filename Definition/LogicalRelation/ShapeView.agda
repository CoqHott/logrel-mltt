...
_⊩⟨_⟩U^_ : (Γ : Con Term) (l : TypeLevel) (l : Level) → Set
Γ ⊩⟨ l ⟩U^ ll = MaybeEmb l (λ l′ → Γ ⊩′⟨ l′ ⟩U^ ll)
...
U-intr : ∀ {l Γ r ll} → Γ ⊩⟨ l ⟩U^ ll → Γ ⊩⟨ l ⟩ Univ r ll ^ [ ! , next ll ]
U-intr (noemb (Uᵣ l< ⊢Γ)) = Uᵣ (Uᵣ l< ⊢Γ)
...
U-elim : ∀ {l Γ r r' l' l''} → Γ ⊩⟨ l ⟩ Univ r l' ^ [ r' , l'' ] → Γ ⊩⟨ l ⟩U^ l'
U-elim (Uᵣ′ _ l′ l< ⊢Γ) = noemb (Uᵣ l< ⊢Γ)
...
U-elim {ι ¹} (emb {l′ = ι ⁰} (Nat.s≤s X) x) with U-elim x
U-elim {ι ¹} (emb {l′ = ι ⁰} (Nat.s≤s X) x) | noemb x₁ = emb (Nat.s≤s X) (noemb x₁)
U-elim {ι ¹} (emb {l′ = ι ⁰} (Nat.s≤s X) x) | emb () x₁
U-elim {∞} (emb {l′ = ι ⁰} (Nat.s≤s X) x) with U-elim x
U-elim {∞} (emb {l′ = ι ⁰} (Nat.s≤s X) x) | noemb x₁ = emb (Nat.s≤s X) (noemb x₁)
U-elim {∞} (emb {l′ = ι ⁰} (Nat.s≤s X) x) | emb () x₁
U-elim {∞} (emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s X)) x) with U-elim x
U-elim {∞} (emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s X)) x) | noemb x₁ = emb (Nat.s≤s (Nat.s≤s X)) (noemb x₁)
U-elim {∞} (emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s X)) x) | emb <l x₁ = emb {l′ = ι ¹} (Nat.s≤s (Nat.s≤s X)) (emb <l x₁)
U-elim {∞} (emb {l′ = ∞} (Nat.s≤s (Nat.s≤s ())) x)

ℕ-elim′ : ∀ {l A Γ ll} → Γ ⊢ A ⇒* ℕ ^ [ ! , ll ]  → Γ ⊩⟨ l ⟩ A ^ [ ! , ll ] → Γ ⊩⟨ l ⟩ℕ A
ℕ-elim′ D (Uᵣ′ _ ⁰ l< ⊢Γ) = ⊥-elim (U≢ℕ (whrDet* (id (univ (univ 0<1 ⊢Γ)) , Uₙ) (D , ℕₙ)))
ℕ-elim′ D (Uᵣ′ _ ¹ l< ⊢Γ) = ⊥-elim (U≢ℕ (whrDet* (id (Uⱼ ⊢Γ) , Uₙ) (D , ℕₙ)))
...
ne-elim′ D neK (Uᵣ′ _ ⁰ l< ⊢Γ) e = ⊥-elim (U≢ne neK (whrDet* (id (univ (univ 0<1 ⊢Γ)) , Uₙ) (D , ne neK)))
ne-elim′ D neK (Uᵣ′ _ ¹ l< ⊢Γ) e = ⊥-elim (U≢ne neK (whrDet* (id (Uⱼ ⊢Γ) , Uₙ) (D , ne neK)))
...
Π-elim′ D (Uᵣ′ _ ⁰ l< ⊢Γ) = ⊥-elim (U≢Π (whrDet* (id (univ (univ 0<1 ⊢Γ)) , Uₙ) (D , Πₙ)))
Π-elim′ D (Uᵣ′ _ ¹ l< ⊢Γ) = ⊥-elim (U≢Π (whrDet* (id (Uⱼ ⊢Γ) , Uₙ) (D , Πₙ)))
...
data ShapeView Γ : ∀ l l′ A B r r' (p : Γ ⊩⟨ l ⟩ A ^ r) (q : Γ ⊩⟨ l′ ⟩ B ^ r') → Set where
  Uᵥ : ∀ {r r' l l′ lA lB} UA UB → ShapeView Γ l l′ (Univ r lA) (Univ r' lB) [ ! , next lA ] [ ! , next lB ] (Uᵣ UA) (Uᵣ UB)
...
data ShapeView₃ Γ : ∀ l l′ l″ A B C r1 r2 r3
  ...
  Uᵥ : ∀ {l l′ l″ r1 r2 r3 l1 l2 l3} UA UB UC → ShapeView₃ Γ l l′ l″ (Univ r1 l1) (Univ r2 l2) (Univ r3 l3)
                                                           [ ! , next l1 ] [ ! , next l2 ] [ ! , next l3 ] (Uᵣ UA) (Uᵣ UB) (Uᵣ UC)
...
combine (Uᵥ UA UB) (Πᵥ (Πᵣ rF F G D ⊢F ⊢G A≡A [F] [G] G-ext) ΠB) =
  ⊥-elim (U≢Π (whnfRed* (red D) Uₙ))
combine (Uᵥ UA UB) (∃ᵥ (∃ᵣ F G D ⊢F ⊢G A≡A [F] [G] G-ext) ∃B) =
  ⊥-elim (U≢∃ (whnfRed* (red D) Uₙ))
...
combine  (∃ᵥ ∃A (∃ᵣ F G D ⊢F ⊢G A≡A [F] [G] G-ext)) (Uᵥ UA UB) =
  ⊥-elim (U≢∃ (whnfRed* (red D) Uₙ))
