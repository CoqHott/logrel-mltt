...
mutual
  ...
  data _⊢_~_↑!_^_ (Γ : Con Term) : (k l A : Term) → TypeLevel → Set where
    ...
    natrec-cong : ∀ {k l h g a₀ b₀ F G lF}
                → Γ ∙ ℕ ^ [ ! , ι ⁰ ] ⊢ F [conv↑] G ^ [ ! , ι lF ]
                → Γ ⊢ a₀ [conv↑] b₀ ∷ F [ zero ] 
                → Γ ⊢ h [conv↑] g ∷ Π ℕ ^ ! ° ⁰ ▹ (F ^ ! ° lF ▹▹ F [ suc (var 0) ]↑ ° lF) ° lF 
                → Γ ⊢ k ~ l ↓! ℕ ^ ι ⁰
    ...
    cast-ΠΠ%! : ∀ {A P A' P' B Q B' Q' t t' e e'}
              → Γ ⊢ Π A ^ % ° ⁰ ▹ P ° ⁰ [conv↑] Π A' ^ % ° ⁰ ▹ P' ° ⁰ ∷ U ⁰
              → Γ ⊢ Π B ^ ! ° ⁰ ▹ Q ° ⁰ [conv↑] Π B' ^ ! ° ⁰ ▹ Q' ° ⁰ ∷ U ⁰
              → Γ ⊢ t [conv↑] t' ∷ Π A ^ % ° ⁰ ▹ P ° ⁰
  ...
  data _⊢_[conv↓]_∷_ (Γ : Con Term) : (t u A : Term) → Set where
    ...
    η-eq      : ∀ {f g F G rF lF lG l}
              ...
              → Function f
              → Function g
              → Γ ∙ F ^ [ rF , ι lF ] ⊢ wk1 f ∘ var 0 [conv↑] wk1 g ∘ var 0 ∷ G 
              → Γ ⊢ f [conv↓] g ∷ Π F ^ rF ° lF ▹ G ° lG
