...
data [Empty]-prop (Γ : Con Term) : (n n′ : Term) → Set where
  ne    : ∀ {n n′} → Γ ⊢ n ∷ Empty ^ % → Γ ⊢ n′ ∷ Empty ^ %  → [Empty]-prop Γ n n′
...
module LogRel (l : TypeLevel) (rec : ∀ {l′} → l′ <∞ l → LogRelKit) where
  ...
  toLevel⊩¹U : {Γ : Con Term} → Γ ⊩¹U → Level
  toLevel⊩¹U (Uᵣ l′ l< ⊢Γ) = toLevel l<
  
  -- Universe type equality
  _⊩¹U_≡_,_ : (Γ : Con Term) (r : Relevance) (B : Term) (l : Level) → Set
  Γ ⊩¹U r ≡ B , l = B PE.≡ Univ r l
  ...
  record _⊩¹U_∷U_/_ {l′} (Γ : Con Term) (t : Term) (r : Relevance) (l< : l′ <∞ l) : Set where
    ...
    field
      A     : Term
      d     : Γ ⊢ t :⇒*: A ∷ (Univ r (toLevel l<))
      typeA : Type A
      A≡A   : Γ ⊢ A ≅ A ∷ Univ r (toLevel l<) ^ ! 
      [t]   : Γ ⊩ t ^ r
      [IdA] : (a a' : Term)
            → Γ ⊩ a ∷ t ^ r / [t]
            → Γ ⊩ a' ∷ t ^ r / [t]
      ...
      [castA] : (B a e : Term)
            → ([B] : Γ ⊩ B ^ r)
            → Γ ⊩ a ∷ t ^ r / [t]
            → Γ ⊩ cast (toLevel l<) t B a e ∷ B ^ r / [B]
  ...
  record _⊩¹U_≡_∷U_/_ {l′} (Γ : Con Term) (t u : Term) (r : Relevance) (l< : l′ <∞ l) : Set where
    ...
    field
      A B   : Term
      d     : Γ ⊢ t :⇒*: A ∷ Univ r (toLevel l<)
      d′    : Γ ⊢ u :⇒*: B ∷ Univ r (toLevel l<)
      typeA : Type A
      typeB : Type B
      A≡B   : Γ ⊢ A ≅ B ∷ Univ r (toLevel l<) ^ !
      [t]   : Γ ⊩ t ^ r
      [u]   : Γ ⊩ u ^ r
      [t≡u] : Γ ⊩ t ≡ u ^ r / [t]
  ...
  mutual
    ...
    -- Term equality of Π-type
    _⊩¹Πirr_≡_∷_/_ : (Γ : Con Term) (t u A : Term) ([A] : Γ ⊩¹Π A ^ %) → Set
    Γ ⊩¹Πirr t ≡ u ∷ A / Πᵣ rF F G D ⊢F ⊢G A≡A [F] [G] G-ext =
          (Γ ⊢ t ∷ Π F ^ rF ▹ G ^ %)
          ×
          (Γ ⊢ u ∷ Π F ^ rF ▹ G ^ %)

    record _⊩¹∃_ (Γ : Con Term) (A : Term) : Set where
      inductive
      eta-equality
      constructor ∃ᵣ
      field
    ...
    -- Π-type equality
    record _⊩¹∃_≡_/_ (Γ : Con Term) (A B : Term) ([A] : Γ ⊩¹∃ A) : Set where
      inductive
      eta-equality
      constructor ∃₌
      open _⊩¹∃_ [A]
    ...
    _⊩¹∃_∷_/_ : (Γ : Con Term) (t A : Term) ([A] : Γ ⊩¹∃ A) → Set
    Γ ⊩¹∃ t ∷ A / ∃ᵣ F G D ⊢F ⊢G A≡A [F] [G] G-ext =
      Γ ⊢ t ∷ ∃ F ▹ G ^ %

    _⊩¹∃_≡_∷_/_ : (Γ : Con Term) (t u A : Term) ([A] : Γ ⊩¹∃ A) → Set
    Γ ⊩¹∃ t ≡ u ∷ A / ∃ᵣ F G D ⊢F ⊢G A≡A [F] [G] G-ext =
          (Γ ⊢ t ∷ ∃ F ▹ G ^ %)
          ×
          (Γ ⊢ u ∷ ∃ F ▹ G ^ %)
...
logRelRec .(ι ¹) 0<1 = LogRel.kit (ι ⁰) (λ ())
logRelRec .∞ 0<∞ = LogRel.kit (ι ⁰) (λ ())
logRelRec .∞ 1<∞ = LogRel.kit (ι ¹) (logRelRecLevel ¹)
