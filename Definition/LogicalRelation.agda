...
natural : ∀ {Γ n} → Natural-prop Γ n → whNf n
natural (sucᵣ _) = sucₙ
natural zeroᵣ = zeroₙ
natural (ne (neNfₜ neK ⊢k k≡k)) = ne (NeutralwhNeutral neK)

-- Natural extraction from term equality WHNF property
split : ∀ {Γ a b} → [Natural]-prop Γ a b → whNf a × whNf b
split (sucᵣ _) = sucₙ , sucₙ
split zeroᵣ = zeroₙ , zeroₙ
split (ne (neNfₜ₌ neK neM k≡m)) = ne (NeutralwhNeutral neK) , ne (NeutralwhNeutral neM)
...
module LogRel (l : TypeLevel) (rec : ∀ {l′} → l′ < l → LogRelKit) where
  ...
  -- Universe type
  record _⊩¹U_ (Γ : Con Term) (A : Term) : Set where
    constructor Uᵣ
    field
      r : Relevance
      l′ : TypeLevel
  ...
  _⊩¹U_≡_/_ : (Γ : Con Term) (A B : Term) ([A] : Γ ⊩¹U A) → Set
  Γ ⊩¹U A ≡ B / [A] = B PE.≡ A
  ...
  mutual
    ...
    _⊩¹_∷_^_/_ : (Γ : Con Term) (t A : Term) (r : Relevance) → Γ ⊩¹ A ^ r → Set
    Γ ⊩¹ t ∷ A ^ .! / Uᵣ UA = Γ ⊩¹U t ∷U (_⊩¹U_.r UA) / _⊩¹U_.l< UA
    Γ ⊩¹ t ∷ A ^ .! / ℕᵣ D = Γ ⊩ℕ t ∷ℕ
    ...
    _⊩¹_≡_∷_^_/_ : (Γ : Con Term) (t u A : Term) (r : Relevance) → Γ ⊩¹ A ^ r → Set
    Γ ⊩¹ t ≡ u ∷ A ^ .! / Uᵣ UA = Γ ⊩¹U t ≡ u ∷U (_⊩¹U_.r UA) / _⊩¹U_.l< UA
...
pattern Uᵣ′ a b c d e = Uᵣ (Uᵣ a b c d e)
...
_⊩′⟨_⟩U_ : (Γ : Con Term) (l : TypeLevel) → Term → Set
