  ...
  mutual
    ...
    record _⊩¹Π_^[_] (Γ : Con Term) (A : Term) (lΠ : Level)  : Set where
      ...
      field
        ...
        [F] : ∀ {ρ} → ρ ∷ ε ⊆ Γ → (⊢Δ : ⊢ ε) → ε ⊩¹ U.wk ρ F ^ [ rF , ι lF ]
        [G] : ∀ {ρ a}
            → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
    ...
    record _⊩¹Π_≡_^[_]/_ (Γ : Con Term) (A B : Term) (lΠ : Level) ([A] : Γ ⊩¹Π A ^[ lΠ ]) : Set where
      ...
      field
        ...
        [F≡F′] : ∀ {ρ}
               → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
               → ε ⊩¹ U.wk ρ F ≡ U.wk ρ F′ ^ [ rF , ι lF ] / [F] [ρ] ⊢Δ
        [G≡G′] : ∀ {ρ a}
               → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
               → ([a] : ε ⊩¹ a ∷ U.wk ρ F ^ [ rF , ι lF ] / [F] [ρ] ⊢Δ)
               → ε ⊩¹ U.wk (lift ρ) G [ a ] ≡ U.wk (lift ρ) G′ [ a ] ^ [ ! , ι lG ] / [G] [ρ] ⊢Δ [a]
      ...
      ∃ λ f → Γ ⊢ t :⇒*: f ∷ Π F ^ rF ° lF ▹ G ° lG ° lΠ ^ ! ^ ι lΠ
            ...
            × (∀ {ρ a b}
              → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
                ...
                ([a≡b] : ε ⊩¹ a ≡ b ∷ U.wk ρ F ^ [ rF , ι lF ] / [F] [ρ] ⊢Δ)
              → ε ⊩¹ U.wk ρ f ∘ a ^ lΠ ≡ U.wk ρ f ∘ b ^ lΠ ∷ U.wk (lift ρ) G [ a ] ^ [ ! , ι lG ] / [G] [ρ] ⊢Δ [a])
            × (∀ {ρ a} → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
              → ([a] : ε ⊩¹ a ∷ U.wk ρ F ^ [ rF , ι lF ] / [F] [ρ] ⊢Δ)
              → ε ⊩¹ U.wk ρ f ∘ a ^ lΠ ∷ U.wk (lift ρ) G [ a ] ^ [ ! , ι lG ] / [G] [ρ] ⊢Δ [a])
    ...
    Γ ⊩¹Π t ≡ u ∷ A ^ l′ / Πᵣ rF lF lG lF≤ lG≤ F G D ⊢F ⊢G A≡A [F] [G] =
      ...
      ×   (∀ {ρ a} → ([ρ] : ρ ∷ ε ⊆ Γ) (⊢Δ : ⊢ ε)
          → ([a] : ε ⊩¹ a ∷ U.wk ρ F ^ [ rF , ι lF ] / [F] [ρ] ⊢Δ)
          → ε ⊩¹ U.wk ρ f ∘ a ^ l′ ≡ U.wk ρ g ∘ a ^ l′ ∷ U.wk (lift ρ) G [ a ] ^ [ ! , ι lG ] / [G] [ρ] ⊢Δ [a])
    -- Issue: Same as above.
