...
data UFull : Term → Set where
  ∃U  : ∀ {r} → UFull (Univ r)
  ∃Π₁ : ∀ {F rF G} → UFull F → UFull (Π F ^ rF ▹ G)
  ∃Π₂ : ∀ {F rF G} → UFull G → UFull (Π F ^ rF ▹ G)

-- Terms cannot contain U.
noU : ∀ {t A rA Γ} → Γ ⊢ t ∷ A ^ rA → ¬ (UFull t)
noU (ℕⱼ x) ()
noU (Emptyⱼ x) ()
...
inverseUniv : ∀ {A rA Γ} → ¬ (UFull A) → Γ ⊢ A ^ rA → Γ ⊢ A ∷ Univ rA ^ !
...
inverseUnivNe : ∀ {A rA Γ} → Neutral A → Γ ⊢ A ^ rA → Γ ⊢ A ∷ Univ rA ^ !
...
inverseUnivEq′ : ∀ {A B rA Γ} → (¬ (UFull A)) ⊎ (¬ (UFull B))
               → Γ ⊢ A ≡ B ^ rA
               → Γ ⊢ A ≡ B ∷ Univ rA ^ !
...
inverseUnivEq : ∀ {A B rA Γ} → Γ ⊢ A ∷ Univ rA ^ ! → Γ ⊢ A ≡ B ^ rA → Γ ⊢ A ≡ B ∷ Univ rA ^ !
