...
type-uniq (∃ⱼ X ▹ X₁) (∃ⱼ Y ▹ Y₁) =
    let _ , enextl , eU = type-uniq X Y
        el = next-inj enextl
    in PE.refl , enextl , refl ((Ugenⱼ (wfTerm Y)))
...
type-uniq (fstⱼ X X₁ X₂) (fstⱼ Y Y₁ Y₂) =
    let _ , _  , ∃≡∃ = type-uniq X₂ Y₂
        -- erF , elF , [F] = type-uniq (univ X) (univ Y)
        F≡F = {!!}  -- , _ = ∃injectivity ∃≡∃ 
    in PE.refl , PE.refl , F≡F
...
type-uniq (Idⱼ X X₁ X₂) (Idⱼ Y Y₁ Y₂) =
    PE.refl , PE.refl , refl (Ugenⱼ (wfTerm X))
type-uniq (Idreflⱼ X) (Idreflⱼ Y) =
    PE.refl , PE.refl , refl (syntacticTerm (Idreflⱼ X))
type-uniq (transpⱼ x x₁ X X₁ X₂ X₃) (transpⱼ x₂ x₃ Y Y₁ Y₂ Y₃) =
    PE.refl , PE.refl , refl (substitution x₁ (singleSubst X₂) (wfTerm X))
